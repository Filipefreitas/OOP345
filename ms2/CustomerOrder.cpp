	...
	for (unsigned int i = 0; i < m_cntItem; i++)
	{
		if (m_listItem[i]->m_itemName == station.getItemName())
		{
			if (station.getQuantity() <= 0)
				os << std::setw(m_widthField) << std::right << "Unable to fill " << m_name << ", " << m_product << " [" << m_listItem[i]->m_itemName << "]" << std
				::endl;
			else
			{
				...
				os << std::setw(m_widthField - 5) << std::right << "Filled " << m_name << ", " << m_product << " [" << m_listItem[i]->m_itemName << "]" << std::endl;
			}
