...
namespace sdds {
	...
	void SongCollection::display(std::ostream& out) const 
	{
		...
		size_t sumTime = accumulate(_collection.begin(), _collection.end(), 0u,
		[](const size_t& sumLength, const Song& song) 
		{
	...
	void SongCollection::sort(string column) 
	{
		if (column == "title") 
		{
			::sort(_collection.begin(), _collection.end(), [](const Song& front,
				const Song& after) {
	...
	{
		for_each(_collection.begin(), _collection.end(),
			[](Song& song) { song._album = song._album == "[None]" ? "" :
			song._album; });
	}
	bool SongCollection::inCollection(string artist) const 
	{
		auto valid = find_if(_collection.begin(), _collection.end(),
			[&artist](const Song& song) {return song._artist == artist; });
